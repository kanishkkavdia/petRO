...
const SLEEP_MS    = 5 * 60 * 1000; // Time until actual sleep 
...
// Upgraded Dance Steps using complex combos (wheels + neck)
const DANCE_STEPS = [
...
async function doDance() {
  toast('💃 Dancing!'); setEmotion('excited'); // Will trigger Firmware 'D'
  for (const [cmd, ms] of DANCE_STEPS) { await bleSend(cmd); if (ms > 0) await sleep(ms); }
  toast('🎉 Done!');
}

async function doWander() {
  toast('🗺 Wandering!'); setEmotion('focused'); // Will trigger Firmware 'N'
  await bleSend('W'); // Trigger firmware scared/wander sequence directly
  await sleep(4000);
  toast('✅ Done');
}
...
function bindSensors() {
  window.addEventListener('devicemotion', (e) => { if(isSleeping || !motionEnabled) return; let acc = e.accelerationIncludingGravity; if(!acc) return; let total = Math.sqrt(acc.x*acc.x + acc.y*acc.y + acc.z*acc.z); if(Math.abs(total - lastAccel) > 16) { setEmotion('dizzy'); resetInactivity(); } lastAccel = total; });
  window.addEventListener('deviceorientation', (e) => { if(isSleeping || !motionEnabled) return; if(Math.abs(e.beta) > 60 || Math.abs(e.gamma) > 60) { if(currentEmotion !== 'afraid') setEmotion('afraid'); resetInactivity(); } });
}
...
AVAILABLE FUNCTIONS:
  ...
  perform_pattern(pattern) -> Executes special movement ('circle', 'rectangle', 'moonwalk', 'spin'). Use if asked to draw shape or dance specifically (like Michael Jackson).

EMOTION HARDWARE TRIGGERING (CRITICAL):
Appending [emotion:NAME] tags directly controls the robot's physical hardware servos to act out complex kinetics!
Always append ONE tag from this list to make the robot physically react:
[emotion:neutral], [emotion:happy], [emotion:excited], [emotion:sad], [emotion:angry], [emotion:focused], [emotion:afraid], [emotion:dizzy]`;
  return p;
}
...
async function openYouTube(query) {
  ...
  try {
    ...
    if (data.items && data.items.length > 0) {
      ...
      // Append fallback link in case iframe blocks
...
function showProp(propName) {
  ...
  if (el) {
    el.style.opacity = '1';
    // Remove after 5 seconds to not clutter
    setTimeout(() => { el.style.opacity = '0'; }, 5000);
  }
}

async function doPattern(pattern) {
  toast(`Executing ${pattern}!`);
  if (!bleConnected) return;
  if (pattern === 'circle') {
    await bleSend('L'); await sleep(3500); await bleSend('S');
  } else if (pattern === 'rectangle') {
    for (let i=0; i<4; i++) { await bleSend('F'); await sleep(1000); await bleSend('R'); await sleep(600); } await bleSend('S');
  } else if (pattern === 'moonwalk') {
    for (let i=0; i<4; i++) { await bleSend('B'); await sleep(500); await bleSend('S'); await sleep(200); }
  } else if (pattern === 'spin') {
    await bleSend('L'); await sleep(1500); await bleSend('S');
  }
}
...
async function doChat(userText, imageDataUrl = null) {
  ...
  try {
    ...
    setEmotion(emotion); speak(reply);
...
function setEmotion(name, force = false) {
  ...
  // Trigger Hardware Emotion Kinetics Map (INTERCEPTS FIRMWARE COMPLEX COMMANDS)
...
// Reset inactivity handles BOTH total sleep, and autonomous idle movement
...
function scheduleIdleMove() {
  idleMoveTimer = setTimeout(() => {
    // If not sleeping, not busy chatting, and BLE is connected -> Do a random slight neck movement
    if (!isSleeping && !isBusy && bleConnected) {
       const idleMoves = ['7', '8', 'E', 'C', 'A', 'V', 'U']; // Neck pan/tilt combos
       bleSend(idleMoves[Math.floor(Math.random() * idleMoves.length)]);
    }
    scheduleIdleMove(); // Re-trigger loop
  }, 10000 + Math.random() * 15000); // Triggers every 10-25 seconds randomly
}
