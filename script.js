...
// BLE Queue Engine Configuration
let bleQueue = [];
...
function applyTheme() {
  ...
  document.getElementById('lRim')?.setAttribute('stroke', t.color); 
  document.getElementById('rRim')?.setAttribute('stroke', t.color);
  document.getElementById('mouthPath')?.setAttribute('fill', t.color);
  
  // Safe visual layer matching logic
  ['dog', 'terminator', 'monkey', 'starwars'].forEach(a => { 
    const el = document.getElementById('theme-' + a); 
    if (el) el.style.opacity = (tId === a) ? '1' : '0'; 
  });
  
  const mg = document.getElementById('mouthGroup');
  if (mg) mg.style.opacity = (tId === 'transformer') ? '0' : '1';
...
function stopBleHeartbeat() { if (bleHeartbeatTimer) { clearInterval(bleHeartbeatTimer); bleHeartbeatTimer = null; } }
...
async function doDance() { 
  toast('💃 Dancing!'); 
  setEmotion('excited'); 
  hardwareActive = true; isMoving = true;
  ...
  isMoving = false; hardwareActive = false;
  toast('🎉 Done!'); 
}
...
function bindSensors() {
  window.addEventListener('devicemotion', (e) => { 
    ...
    const now = performance.now(); if (now - lastSensorProcessTime < 100) return; lastSensorProcessTime = now;
...
function buildSystemPrompt() {
  ...
  p += `\nAVAILABLE FUNCTIONS:\n  move_forward(sec), move_backward(sec), turn_left(sec), turn_right(sec), stop_robot(), dance(), nod_head(times), wander(), capture_photo(), search_youtube(query, is_entertainment), call_contact(num), show_prop(prop_name), perform_pattern(pattern)\n\nEMOTION HARDWARE TRIGGERING: Append [emotion:NAME] (e.g. [emotion:happy]) to trigger routines!`;
  return p;
}
...
const TOOL_DECLARATIONS = [
  ...
  { name:'search_youtube', parameters:{type:'OBJECT',properties:{query:{type:'STRING'}, is_entertainment:{type:'BOOLEAN'}},required:['query']} },
  { name:'call_contact', parameters:{type:'OBJECT',properties:{ phone_number: {type: 'STRING'} }, required:['phone_number']} },
  { name:'show_prop', parameters:{type:'OBJECT',properties:{ prop_name: {type: 'STRING'} }, required:['prop_name']} },
  { name:'perform_pattern', parameters:{type:'OBJECT',properties:{ pattern: {type: 'STRING'} }, required:['pattern']} }
];
...
function startGroove() {
  ...
  let step = 0; const grooveMoves = ['V', '1', '2', 'U', '7', '8', '3', '4']; 
...
async function openYouTube(query, isEntertainment = false) {
  ...
  try {
    ...
    if (data.items && data.items.length > 0) {
      ...
      if (isYtApiReady) {
          if (!ytPlayer) {
              ytPlayer = new YT.Player('ytPlayerDiv', {
                  ...
                  events: { 'onStateChange': (e) => { if (e.data == YT.PlayerState.ENDED) closeYouTube(); } }
              });
      ...
      isVideoPlaying = true;
      if (alwaysOnMic) { stopAllRecog(); micSuspendedForVideo = true; }
      if (isEntertainment || query.toLowerCase().includes('music')) startGroove();
    } else { toast('❌ No video found.'); }
  } catch (error) { toast('❌ YouTube failed.'); }
}
...
function closeYouTube() { 
  ...
  if (micSuspendedForVideo) { alwaysOnMic = true; startBgListen(); updateMicBadge('wake'); micSuspendedForVideo = false; }
}

function showProp(propName) {
  ['apple', 'book', 'dumbbell', 'laptop'].forEach(p => {
    const el = document.getElementById('prop-' + p); if (el) el.style.opacity = '0';
  });
...
async function executeTools(toolCalls) {
  for (const tc of toolCalls) {
    const args = tc.args || {};
    switch(tc.name) {
      case 'move_forward':  bleSend('F'); await sleep((args.duration_seconds || 1) * 1000); bleSend('S'); break;
      case 'move_backward': bleSend('B'); await sleep((args.duration_seconds || 1) * 1000); bleSend('S'); break;
      case 'turn_left':     bleSend('L'); await sleep((args.duration_seconds || 0.5) * 1000); bleSend('S'); break;
      case 'turn_right':    bleSend('R'); await sleep((args.duration_seconds || 0.5) * 1000); bleSend('S'); break;
...
async function sendChat() {
  ...
  if (!attachedImage && visionKw.some(kw => text.toLowerCase().includes(kw))) { attachedImage = captureSnapshot(); if (attachedImage) appendImageMsg('user', attachedImage); } else if (attachedImage) { appendImageMsg('user', attachedImage); }
...
async function doChat(userText, imageDataUrl = null) {
  ...
  } catch(e) { removeTyping(typEl); appendMsg('bot', `❌ ${e.message}`, []); setEmotion('sad'); speak('Error processing.'); } finally { isBusy = false; document.getElementById('sendBtn').disabled = false; }
}
function addToHistory(role, text, imageBase64 = null) { chatHistory.push({ role, text, imageBase64 }); if (chatHistory.length > MAX_HISTORY) chatHistory.splice(0, chatHistory.length - MAX_HISTORY); updateMemoryPill(); }
function clearChat() { chatHistory = []; document.getElementById('messages').innerHTML = `<div class="msg bot"><div class="msg-bubble">Fresh start! ✨</div></div>`; updateMemoryPill(); }
function updateMemoryPill() { document.getElementById('memoryPill').textContent = `Memory: ${chatHistory.length} / ${MAX_HISTORY} msgs`; }

// ════════════════════════════════════════════════════════
// CAMERA MECHANICS
// ════════════════════════════════════════════════════════
async function initCamera() { try { videoStream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: 'user' } }); document.getElementById('webcamView').srcObject = videoStream; } catch(e) { console.warn('Camera blocked:', e); } }
...
function autoCapture() { const dataUrl = captureSnapshot(); if (dataUrl) { appendImageMsg('bot', dataUrl); doChat('Describe what you see!', dataUrl); } }
...
function updateUploadPreview() { const bar = document.getElementById('uploadPreviewBar'), img = document.getElementById('previewImg'); if (bar && img) { img.src = currentUploadedImage || ''; bar.style.display = currentUploadedImage ? 'flex' : 'none'; } }
function appendImageMsg(role, dataUrl) { const c = document.getElementById('messages'), el = document.createElement('div'); el.className = `msg ${role}`; const bub = document.createElement('div'); bub.className = 'msg-bubble'; bub.style.padding = '4px'; const img = document.createElement('img'); img.src = dataUrl; img.style.cssText = 'max-width:100px;border-radius:10px;display:block;'; bub.appendChild(img); el.appendChild(bub); c.appendChild(el); c.scrollTop = c.scrollHeight; }
...
function speak(text) {
  if (!synth || isVideoPlaying) return; synth.cancel(); 
  ...
  if (theme === 'terminator') { pitch = 0.4; rate = 0.9; } else if (theme === 'monkey') { pitch = 1.5; rate = 1.25; }
  ...
  utt.onend = () => { ttsActive = false; updateTTSBtn(); animateMouth(false); if (!isBusy) startFollowUp(); }; 
...
function runEyeLoop() { function f() { Object.keys(eyeOff).forEach(k => eyeOff[k] += (eyeTgt[k] - eyeOff[k]) * 0.25); if (eyeEls.lIris) { eyeEls.lIris.setAttribute('cx', 120 + eyeOff.lx); eyeEls.lIris.setAttribute('cy', 110 + eyeOff.ly); eyeEls.lPupil.setAttribute('cx', 120 + eyeOff.lx * 1.2); eyeEls.lPupil.setAttribute('cy', 110 + eyeOff.ly * 1.2); eyeEls.lHL1.setAttribute('cx', 110 + eyeOff.lx * 1.3); eyeEls.lHL1.setAttribute('cy', 100 + eyeOff.ly * 1.3); } if (eyeEls.rIris) { eyeEls.rIris.setAttribute('cx', 280 + eyeOff.rx); eyeEls.rIris.setAttribute('cy', 110 + eyeOff.ry); eyeEls.rPupil.setAttribute('cx', 280 + eyeOff.rx * 1.2); eyeEls.rPupil.setAttribute('cy', 110 + eyeOff.ry * 1.2); eyeEls.rHL1.setAttribute('cx', 270 + eyeOff.rx * 1.3); eyeEls.rHL1.setAttribute('cy', 100 + eyeOff.ry * 1.3); } requestAnimationFrame(f); } f(); }
function scheduleBlink() { if(blinkTimer) clearTimeout(blinkTimer); blinkTimer = setTimeout(async () => { if(!isSleeping && !['dizzy','afraid'].includes(currentEmotion)) { await setLids(90, 90); await sleep(90); await setLids(emotions[currentEmotion]?.lids || 0, emotions[currentEmotion]?.lids || 0); } scheduleBlink(); }, 2500 + Math.random() * 3000); }
function scheduleEyeMove() { setTimeout(() => { if(!isSleeping && Math.random() > 0.3 && !['focused'].includes(currentEmotion)) { const mx = (Math.random() - 0.5) * 16, my = (Math.random() - 0.5) * 10; eyeTgt = { lx:mx, ly:my, rx:mx, ry:my }; } scheduleEyeMove(); }, 1500 + Math.random() * 2000); }
...
function setEmotion(emName, force = false) {
  ...
  if (bleConnected && !force) bleSend(`E${emName.substring(0,2).toUpperCase()}`);
  if (!['neutral','sleeping'].includes(currentEmotion) && !force) emotionResetTimer = setTimeout(() => setEmotion('neutral'), 6000);
}
...
function toggleMicMode() {
  ...
  if (!alwaysOnMic && micState === 'off') { alwaysOnMic = true; startBgListen(); toast('🎤 Always-On Mode'); } 
  else if (alwaysOnMic && micState === 'wake') { stopAllRecog(); try { cmdRecog.start(); micState = 'listening'; } catch{} alwaysOnMic = false; } 
  else { alwaysOnMic = false; stopAllRecog(); updateMicBadge('off'); }
}
...
// ════════════════════════════════════════════════════════
// SLEEP / AUTO RECOVER CONSOLE
// ════════════════════════════════════════════════════════
function resetInactivity() { clearTimeout(inactTimer); inactTimer = setTimeout(goToSleep, SLEEP_MS); if (isSleeping) wakeUp(); }
function goToSleep() { if (isSleeping) return; isSleeping = true; stopTTS(); stopAllRecog(); stopGroove(); setEmotion('sleeping', true); toast('💤 Sleep Mode'); bleSend('Z'); }
function wakeUp() { if (!isSleeping) return; isSleeping = false; setEmotion('neutral', true); toast('☀️ Awake!'); if (alwaysOnMic && !isVideoPlaying) startBgListen(); bleSend('A'); resetInactivity(); }

function startFollowUp() {
    if (isBusy || isVideoPlaying || followUpActive) return;
    const config = document.getElementById('followUpSelect').value || '0'; if (config === '0') return;
    clearTimeout(followUpTimeout);
    followUpTimeout = setTimeout(async () => {
        if (isBusy || isVideoPlaying || ttsActive) return; followUpActive = true;
        const apiKey = getApiKey(); if (!apiKey) { followUpActive = false; return; }
        try {
            const body = { system_instruction: { parts: [{ text: buildSystemPrompt() }] }, contents: [{role: 'user', parts:[{text: "Ask a short, engaging, single-sentence question to continue conversation."}]}], generationConfig: { temperature: 0.8, maxOutputTokens: 60 } };
            const resp = await fetch(`${GEMINI_URL}?key=${apiKey}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
            if (resp.ok) {
                const data = await resp.json(); const r = data?.candidates?.[0]?.content?.parts?.[0]?.text || '';
                if (r && !isBusy && !isVideoPlaying) { appendMsg('bot', r.trim()); speak(r.trim()); }
            }
        } catch (e) {} finally { followUpActive = false; }
...
function appendMsg(role, text, tools = []) {
  ...
  const lbl = document.createElement('div'); lbl.className = 'msg-label'; lbl.textContent = role === 'user' ? 'User' : 'petRO'; el.appendChild(lbl);
...
window.addEventListener('DOMContentLoaded', () => {
  initEyes(); initSTT(); initCamera(); loadPersonalization(); updateKeyBadge();
  document.getElementById('userInput').addEventListener('keydown', (e) => { if (e.key === 'Enter') sendChat(); });
  ...
  setInterval(() => {
    ...
    const p = ['circle', 'moonwalk', 'spin']; doPattern(p[Math.floor(Math.random() * p.length)]);
  }, 20000);
});
